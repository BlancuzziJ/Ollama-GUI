...
class GPUInfo:
    ...
    def _detect_hardware(self):
        ...
        if not GPU_UTIL_AVAILABLE and not PSUTIL_AVAILABLE:
            logging.info("GPU detection libraries not installed. Using basic detection.")
        self._detect_gpus()
    ...
    def _detect_gpus(self):
        ...
        if not self.gpu_data:
            ...
            if not GPU_UTIL_AVAILABLE:
                fallback_note += " - Install requirements-gpu.txt for enhanced detection"
                    ...
                    if line.strip():
                        ...
                        if len(parts) >= 4:
                            try:
                                ...
                                ram_mb = ram_bytes / (1024 * 1024) if ram_bytes > 0 else 0
                                
                                # Skip if already detected by GPUtil
                                gpu_name = parts[3].strip()
                                if not any(gpu['name'] == gpu_name for gpu in self.gpu_data):
                                    self.gpu_data.append({
                                        'name': gpu_name,
                                        'memory_total': ram_mb,
                                        'memory_used': 0,
            ...
            if result.returncode == 0:
                ...
                for display in displays:
                    ...
                    if 'MB' in vram:
                        ...
                        vram_mb = int(float(vram.replace(' GB', '').replace(',', '')) * 1024)
            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    if 'VGA compatible controller' in line or 'Display controller' in line:
                        name = line.split(': ', 1)[-1] if ': ' in line else 'Unknown GPU'
    ...
    def get_ai_recommendations(self) -> Dict:
        ...
        if self.system_ram < 16:
            recommendations['limitations'].append(f'System RAM: {self.system_ram:.1f}GB (16GB+ recommended)')
