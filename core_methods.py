...
try:
    from gpu_info import (
        get_gpu_info,
        format_gpu_info_for_display,
        check_gpu_dependencies,
        get_dependency_install_message,
    )
...
def refresh_models_display(self):
    ...
    if not self.models:
        self.models_listbox.insert(
            "1.0", "No models available. Pull a model to get started."
        )
    else:
    ...
    def pull_thread():
        try:
            ...
            def progress_callback(data):
                ...
                self.root.after(
                    0, lambda: self.progress_label.configure(text=f"{status}...")
                )
            ...
            error_msg = str(e)
            self.root.after(
                0, lambda: self.progress_label.configure(text=f"Error: {error_msg}")
            )
        finally:
            self.root.after(
                0, lambda: self.pull_btn.configure(text="Pull", state="normal")
            )
...
def delete_selected_model(self):
    ...
    messagebox.showinfo(
        "Feature", "Model deletion will be implemented with proper selection UI"
    )
...
def load_selected_session(self):
    ...
    messagebox.showinfo(
        "Feature", "Session loading will be implemented with proper selection UI"
    )
...
def show_gpu_info(self):
    ...
    def load_gpu_info():
        try:
            ...
            self.root.after(
                0, lambda: messagebox.showerror("GPU Detection Error", error_msg)
            )
