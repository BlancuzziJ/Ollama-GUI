    ...
    def clear_logs(self) -> bool:
        ...
        try:
            ...
            for handler in root_logger.handlers[:]:
                if hasattr(handler, "baseFilename") and "security.log" in str(
    ...
    def validate_url(self, url: str) -> bool:
        ...
        try:
            if not url or len(url) > self.MAX_URL_LENGTH:
                self.logger.warning(
                    f"URL validation failed: Invalid length - {len(url) if url else 0}"
                )
                return False
            ...
            if parsed.scheme.lower() not in self.ALLOWED_SCHEMES:
                self.logger.warning(
                    f"URL validation failed: Invalid scheme - {parsed.scheme}"
                )
                return False
            ...
            if self.contains_dangerous_patterns(url):
                self.logger.warning(
                    f"URL validation failed: Contains dangerous patterns - {url}"
                )
                return False
            ...
            if not self.validate_domain(parsed.netloc):
                self.logger.warning(
                    f"URL validation failed: Invalid domain - {parsed.netloc}"
                )
                return False
    ...
    def validate_message_input(self, message: str) -> bool:
        ...
        try:
            ...
            if len(message) > self.MAX_MESSAGE_LENGTH:
                self.logger.warning(
                    f"Message validation failed: Too long - {len(message)}"
                )
                return False
    ...
    def validate_file_path(self, file_path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            if len(path.name) > self.MAX_FILENAME_LENGTH:
                self.logger.warning(
                    f"File path validation failed: Name too long - {path.name}"
                )
                return False
            ...
            if ".." in str(path) or str(path).startswith("/"):
                self.logger.warning(
                    f"File path validation failed: Path traversal attempt - {path}"
                )
                return False
            ...
            try:
                ...
                self.logger.warning(
                    f"File path validation failed: Outside user directory - {path}"
                )
                return False
            ...
            if isinstance(arg, str) and len(arg) > 0:
                if security.contains_dangerous_patterns(arg):
                    security.log_security_event(
                        "Dangerous input blocked", {"input": arg[:100]}
                    )
