...
def create_shamollama_icon():
    ...
    draw.ellipse([center-radius, center-radius+5, center+radius, center+radius+5], 
                 fill=(70, 130, 180, 255), outline=(100, 160, 210, 255), width=2)
    
    # Guitar neck
    neck_width = 4
    neck_height = 25
    draw.rectangle([center-neck_width//2, center-radius-neck_height, 
                   center+neck_width//2, center-radius], 
                  fill=(139, 69, 19, 255))  # Brown
    
    # Guitar head
    draw.ellipse([center-6, center-radius-neck_height-8, 
                 center+6, center-radius-neck_height+2], 
                fill=(139, 69, 19, 255))
    
    # Sound hole
    draw.ellipse([center-8, center-3, center+8, center+13], 
                fill=(20, 20, 30, 255))
    
    # Strings
    for i in range(3):
        y_pos = center - radius - neck_height + (i + 1) * 6
        draw.line([center-1, y_pos, center-1, center+radius], 
                 fill=(220, 220, 220, 180), width=1)
        draw.line([center+1, y_pos, center+1, center+radius], 
                 fill=(220, 220, 220, 180), width=1)
    ...
    # Save as ICO file
    img.save('shamollama_icon.ico', format='ICO', sizes=[(16,16), (32,32), (48,48), (64,64)])
