...
class OllamaAPI:
                    ...
                    if isinstance(model, dict) and "name" in model:
                        if security.validate_model_name(model["name"]):
                            ...
                            security.log_security_event(
                                "Invalid model name detected", {"model": model["name"]}
                            )
    ...
    def pull_model(
        ...
        if not security.validate_model_name(model_name):
            security.log_security_event(
                "Invalid model name for pull", {"model": model_name}
            )
            return False
    ...
    def delete_model(self, model_name: str) -> bool:
        ...
        if not security.validate_model_name(model_name):
            security.log_security_event(
                "Invalid model name for deletion", {"model": model_name}
            )
            return False
        ...
        try:
            ...
            security.log_security_event(
                "Failed to delete model", {"model": model_name, "error": str(e)}
            )
            return False
            ...
            if response.status_code == 200:
                if stream_callback:
                    ...
                    if hide_thinking:
                        ...
                        response_content = self._filter_thinking_content(
                            response_content
                        )
    ...
    def _filter_thinking_content(self, content: str) -> str:
        ...
        for pattern in patterns:
            filtered_content = re.sub(
                pattern, "", filtered_content, flags=re.DOTALL | re.IGNORECASE
            )
...
class ChatManager:
    ...
    def export_session(self, filepath: str, session_index: int = -1):
        ...
        session = (
            self.chat_history[session_index]
            if session_index >= 0
            else {
...
class ShamaOllamaGUI:
    ...
    def create_sidebar(self):
        ...
        title_label = ctk.CTkLabel(
            self.sidebar, text="ShamaOllama", font=ctk.CTkFont(size=20, weight="bold")
        )
        ...
        self.new_chat_btn = ctk.CTkButton(
            self.sidebar, text="New Chat", command=self.new_chat, height=35
        )
        ...
        for i, (text, command) in enumerate(nav_buttons):
            btn = ctk.CTkButton(
                nav_frame, text=text, command=command, height=35, corner_radius=6
            )
        ...
        self.theme_btn = ctk.CTkButton(
            self.sidebar, text="🌙 Dark Theme", command=self.toggle_theme, height=30
        )
    ...
    def create_chat_panel(self):
        ...
        self.chat_display = ctk.CTkTextbox(
            self.chat_panel, wrap="word", font=ctk.CTkFont(size=14), state="disabled"
        )
        ...
        self.message_input = ctk.CTkTextbox(
            input_frame, height=80, font=ctk.CTkFont(size=14), wrap="word"
        )
        self.message_input.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")

        # Send button
        self.send_btn = ctk.CTkButton(
            input_frame, text="Send", command=self.send_message, width=80, height=35
        )
        ...
        self.add_chat_message(
            "System",
            "Welcome to ShamaOllama! Select a model from the sidebar to start chatting.",
            "system",
        )
    ...
    def create_models_panel(self):
        ...
        self.models_count_label = ctk.CTkLabel(
            header_frame, text="Loading models...", font=ctk.CTkFont(size=12)
        )
        ...
        ctk.CTkLabel(
            pull_frame, text="📥 Pull Model:", font=ctk.CTkFont(weight="bold")
        ...
        quick_pull_frame.grid(
            row=1, column=0, columnspan=3, padx=5, pady=5, sticky="ew"
        )

        ctk.CTkLabel(
            quick_pull_frame, text="Quick Pull:", font=ctk.CTkFont(size=12)
        ...
        self.progress_percentage = ctk.CTkLabel(
            progress_frame, text="", font=ctk.CTkFont(size=10)
        )
    ...
    def create_history_panel(self):
        ...
        self.load_session_btn = ctk.CTkButton(
            controls_frame, text="Load Session", command=self.load_selected_session
        )
        self.load_session_btn.grid(row=0, column=0, padx=5, pady=5)

        self.export_btn = ctk.CTkButton(
            controls_frame, text="Export Session", command=self.export_selected_session
        )
    ...
    def create_settings_panel(self):
        ...
        # Ollama URL
        ctk.CTkLabel(form_frame, text="Ollama URL:").grid(
            row=0, column=0, padx=10, pady=5, sticky="w"
        )
        self.url_entry = ctk.CTkEntry(
            form_frame, placeholder_text="http://localhost:11434"
        )
        ...
        self.test_btn = ctk.CTkButton(
            form_frame, text="Test Connection", command=self.test_connection_manual
        )
        ...
        self.autosave_check = ctk.CTkCheckBox(
            form_frame, text="Auto-save chat sessions", variable=self.autosave_var
        )
        self.autosave_check.grid(
        ...
        # Max history
        ctk.CTkLabel(form_frame, text="Max history entries:").grid(
        ...
        self.security_logging_check.grid(
            row=1, column=0, columnspan=2, padx=10, pady=5, sticky="w"
        )
        ...
        self.input_validation_check.grid(
            row=2, column=0, columnspan=2, padx=10, pady=5, sticky="w"
        )
        ...
        self.url_validation_check.grid(
        ...
        self.memory_enabled_var = ctk.BooleanVar(
            value=self.chat_manager.memory_manager.memory.get("enabled", True)
        )
        ...
        self.memory_enabled_check.grid(
        ...
        # Name
        ctk.CTkLabel(memory_frame, text="Your Name:").grid(
        ...
        ctk.CTkLabel(memory_frame, text="What to call you:").grid(
            row=3, column=0, padx=10, pady=5, sticky="w"
        )
        self.memory_call_name_entry = ctk.CTkEntry(
            memory_frame, placeholder_text="John, Mr. Doe, Boss, etc."
        )
        ...
        # Role
        ctk.CTkLabel(memory_frame, text="Your Role:").grid(
            row=4, column=0, padx=10, pady=5, sticky="w"
        )
        self.memory_role_entry = ctk.CTkEntry(
            memory_frame, placeholder_text="Software Developer"
        )
        self.memory_role_entry.grid(row=4, column=1, padx=10, pady=5, sticky="ew")
        self.memory_role_entry.insert(0, memory_info.get("role", ""))

        # Company
        ctk.CTkLabel(memory_frame, text="Company:").grid(
            row=5, column=0, padx=10, pady=5, sticky="w"
        )
        self.memory_company_entry = ctk.CTkEntry(
            memory_frame, placeholder_text="Acme Corp"
        )
        ...
        # Location
        ctk.CTkLabel(memory_frame, text="Location:").grid(
            row=6, column=0, padx=10, pady=5, sticky="w"
        )
        self.memory_location_entry = ctk.CTkEntry(
            memory_frame, placeholder_text="New York, NY"
        )
        ...
        # Preferences
        ctk.CTkLabel(memory_frame, text="Preferences:").grid(
            row=7, column=0, padx=10, pady=5, sticky="w"
        )
        self.memory_preferences_entry = ctk.CTkEntry(
            memory_frame, placeholder_text="Concise answers, Python preferred"
        )
        self.memory_preferences_entry.grid(
        ...
        ctk.CTkLabel(memory_frame, text="Additional Context:").grid(
            row=8, column=0, padx=10, pady=5, sticky="nw"
        )
        ...
        self.memory_context_text.insert(
            "0.0", self.chat_manager.memory_manager.memory.get("context_notes", "")
        )

        # Custom system prompt
        ctk.CTkLabel(memory_frame, text="Custom Instructions:").grid(
            row=9, column=0, padx=10, pady=5, sticky="nw"
        )
        ...
        self.memory_system_text.insert(
            "0.0", self.chat_manager.memory_manager.memory.get("system_prompt", "")
        )
        ...
        self.save_settings_btn = ctk.CTkButton(
            form_frame, text="Save Settings", command=self.save_settings
        )
    ...
    def create_about_panel(self):
        ...
        app_title = ctk.CTkLabel(
            scroll_frame, text="ShamaOllama", font=ctk.CTkFont(size=28, weight="bold")
        )
        ...
        version_label = ctk.CTkLabel(
            scroll_frame, text="Version 1.0.0", font=ctk.CTkFont(size=14)
        )
    ...
    def create_status_bar(self):
        ...
        self.status_label = ctk.CTkLabel(
            self.status_frame, text="Ready", font=ctk.CTkFont(size=12)
        )
        self.status_label.grid(row=0, column=0, padx=10, pady=5, sticky="w")

        self.connection_label = ctk.CTkLabel(
            self.status_frame, text="⚫ Disconnected", font=ctk.CTkFont(size=12)
        )
...
Note: This feature works best with models specifically designed to show thinking processes. Regular models are not affected by this setting."""
    ...
    def refresh_models(self):
        ...
        def refresh():
            try:
                ...
                # Update dropdown
                self.root.after(
                    0, lambda: self.model_dropdown.configure(values=model_names)
                )
                ...
                self.root.after(
                    ...
                    lambda: self.status_label.configure(
                        text=f"Found {len(self.models)} models"
                    ),
                )
            except Exception as e:
                error_msg = str(e)
                self.root.after(
                    0,
                    lambda: self.status_label.configure(
                        text=f"Error loading models: {error_msg}"
                    ),
                )
    ...
    def create_model_entry(self, index: int, model: Dict):
        ...
        tag_label = ctk.CTkLabel(
            info_frame, text=f"🏷️ {tag}", font=ctk.CTkFont(size=11), anchor="w"
        )
        ...
        size_label = ctk.CTkLabel(
            info_frame, text=size_text, font=ctk.CTkFont(size=10), anchor="w"
        )
    ...
    def on_model_dropdown_selected(self, model_name: str):
        ...
        if model_name != "No models available":
            ...
            self.add_chat_message(
                "System", f"Switched to model: {model_name}", "system"
            )
    ...
    def new_chat(self):
        ...
        self.add_chat_message(
            "System", "New chat session started. How can I help you?", "system"
        )
    ...
    def add_chat_message(self, role: str, content: str, msg_type: str = "normal"):
        ...
        if msg_type == "system":
            ...
            self.chat_display.insert(
                "end", f"[{timestamp}] {model_name}: ", "assistant_role"
            )
            self.chat_display.insert("end", f"{content}\n\n", "assistant_msg")
        ...
        def chat_thread():
            try:
                ...
                def stream_callback(chunk):
                    ...
                    self.root.after(
                        0, lambda c=chunk: self.update_streaming_response(c)
                    )

                full_response = self.api.chat(
                    self.current_model,
                    messages,
                    stream_callback,
                    self.hide_thinking_var.get(),
                )
                ...
                if not full_response and not self.current_response:
                    ...
                    self.root.after(
                        0,
                        lambda: self.status_label.configure(
                            text="Fallback to non-streaming mode..."
                        ),
                    )
                    full_response = self.api.chat(
                        self.current_model, messages, None, self.hide_thinking_var.get()
                    )
                    self.root.after(
                        0, lambda: self.add_chat_message("assistant", full_response)
                    )
                    self.chat_manager.add_message(
                        "assistant", full_response, self.current_model
                    )
                else:
                ...
                self.root.after(
                    0, lambda: self.add_chat_message("System", error_msg, "system")
                )
                self.root.after(0, self.finish_chat_response)
    ...
    def animate_typing_indicator(self):
        ...
        indicators = ["●", "● ●", "● ● ●", "● ●", "●", "", ""]
        current_time = int(time.time() * 2) % len(
            indicators
    ...
    def update_streaming_response(self, chunk):
        ...
        if chunk and self.response_start_pos:
            ...
            self.status_label.configure(
                text=f"📝 Streaming... {word_count} words ({response_length} chars)"
            )
    ...
    def pull_model(self):
        ...
        if not security.validate_model_name(model_name):
            messagebox.showerror(
                "Invalid Model",
                "Invalid model name. Please use only alphanumeric characters, hyphens, underscores, colons, and dots.",
            )
            return
            ...
            try:
                ...
                def progress_callback(data):
                    ...
                    if "total" in data and "completed" in data and data["total"] > 0:
                        ...
                        self.root.after(
                            0,
                            lambda: self.progress_percentage.configure(
                                text=f"{percentage}%"
                            ),
                        )
                    ...
                    self.root.after(
                        0, lambda: self.progress_label.configure(text=display_status)
                    )
                ...
                if success:
                    self.root.after(
                        ...
                        lambda: self.progress_label.configure(
                            text=f"✅ Successfully pulled {model_name}"
                        ),
                    )
                    self.root.after(
                        0, lambda: self.progress_percentage.configure(text="100%")
                    )
                    ...
                    security.log_security_event(
                        "Model pull completed", {"model": model_name}
                    )
                else:
                    self.root.after(
                        0,
                    ...
                    self.root.after(
                        ...
                        lambda: messagebox.showerror(
                            "Pull Failed",
                            f"Failed to pull model '{model_name}'. Please check the model name and try again.",
                        ),
                    )
                    security.log_security_event(
                        "Model pull failed", {"model": model_name}
                    )

            except Exception as e:
                error_msg = f"Error pulling model: {str(e)}"
                self.root.after(
                    0, lambda: self.progress_label.configure(text="❌ Pull error")
                )
                self.root.after(0, lambda: messagebox.showerror("Error", error_msg))
                security.log_security_event(
                    "Model pull error", {"model": model_name, "error": str(e)}
                )
            finally:
                self.root.after(
                    0,
                    lambda: self.pull_btn.configure(text="Pull Model", state="normal"),
                )
        ...
        def delete_models():
            ...
            for model_name in list(self.selected_models):
                ...
                if self.api.delete_model(model_name):
                    ...
                    security.log_security_event(
                        "Model deletion failed", {"model": model_name}
                    )
            ...
            if failed_models:
                ...
                self.root.after(
                    0,
                    lambda: messagebox.showinfo(
                        "Success", f"Successfully deleted {deleted_count} models."
                    ),
                )
    ...
    def copy_selected_model_names(self):
        ...
        messagebox.showinfo(
            "Copied", f"Copied {len(self.selected_models)} model names to clipboard."
        )
    ...
    def show_selected_model_info(self):
        ...
        info_text = ctk.CTkTextbox(
            info_window, wrap="word", font=ctk.CTkFont(family="Courier", size=11)
        )
        ...
        for model_name in sorted(self.selected_models):
            ...
            if model_info:
                ...
                info_content += (
                    f"   Digest: {model_info.get('digest', 'N/A')[:20]}...\n"
                )
        ...
        total_size = sum(
            m.get("size", 0) for m in self.models if m["name"] in self.selected_models
        )
    ...
    def manage_model_tags(self):
        ...
        if not self.selected_models:
            messagebox.showwarning(
                "No Selection", "Please select models to manage tags."
            )
            return
        ...
        for i, model_name in enumerate(sorted(self.selected_models)):
            ...
            # Find related models
            related = [
                m["name"] for m in self.models if m["name"].startswith(base_name + ":")
            ]
            if len(related) > 1:
                ctk.CTkLabel(
                    model_frame,
            ...
            try:
                with open(filename, "w", encoding="utf-8") as f:
                    ...
                    for model_name in sorted(model_names):
                        model_info = next(
                            (m for m in self.models if m["name"] == model_name), None
                        )
                        if model_info:
                            f.write(f"Model: {model_name}\n")
                            f.write(
                                f"Size: {model_info.get('size', 0) / (1024**3):.2f} GB\n"
                            )
                            f.write(
                ...
                messagebox.showinfo(
                    "Export Success", f"Tag report exported to {filename}"
                )
            except Exception as e:
                messagebox.showerror(
                    "Export Failed", f"Failed to export tag report: {e}"
                )
    ...
    def test_connection_manual(self):
        ...
        def test():
            ...
            if is_connected:
                self.root.after(
                    0, lambda: messagebox.showinfo("Success", "Connection successful!")
                )
                security.log_security_event(
                    "Manual connection test successful", {"url": url}
                )
            else:
                self.root.after(
                    0, lambda: messagebox.showerror("Error", "Cannot connect to Ollama")
                )
                security.log_security_event(
                    "Manual connection test failed", {"url": url}
                )
    ...
    def view_security_logs(self):
        ...
        try:
            ...
            log_text = ctk.CTkTextbox(
                log_window, wrap="word", font=ctk.CTkFont(family="Courier", size=10)
            )
        ...
        if result:
            try:
                ...
                if success:
                    ...
                    try:
                        security.log_security_event(
                            "Security logs cleared", {"user_action": True}
                        )
                    except:
                    ...
                    messagebox.showerror(
                        "Error",
                        "Failed to clear security logs. The log file may be in use.",
                    )
            except Exception as e:
                messagebox.showerror(
                    "Error", f"Failed to clear security logs: {str(e)}"
                )
